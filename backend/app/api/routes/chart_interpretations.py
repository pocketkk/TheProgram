"""
Chart interpretation endpoints for AI-generated descriptions
"""
...
async def get_chart_interpretations(
    ...
    query = db.query(ChartInterpretation).filter(ChartInterpretation.chart_id == chart_id)
...
async def get_interpretation(
    ...
    Raises:
        ...
        HTTPException 403: If interpretation's chart doesn't belong to user
    """
    interp = db.query(ChartInterpretation).filter(
        ChartInterpretation.id == interpretation_id
    ).first()
    ...
    # Verify ownership through chart
    chart = db.query(Chart).filter(Chart.id == interp.chart_id).first()
    if not chart or chart.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to access this interpretation"
        )
...
async def update_interpretation(
    ...
    if not chart or chart.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to update this interpretation"
        )
...
async def delete_interpretation(
    ...
    if not chart or chart.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to delete this interpretation"
        )
