...
class PythonBackendManager {
    constructor(port = 8000) {
        ...
        this.isDev = process.env.NODE_ENV !== 'production' && !electron_1.app.isPackaged;
        this.port = port;
    }
    /**
    ...
    async start() {
        return new Promise((resolve, reject) => {
            ...
            const pythonPath = this.isDev
                ? path_1.default.join(backendDir, 'test_venv', 'bin', 'python3') // Use venv Python in dev mode
                : path_1.default.join(process.resourcesPath, 'backend', 'backend'); // Bundled executable in prod
            ...
            const env = {
                ...
                USER_DATA_DIR: electron_1.app.getPath('userData'), // SQLite database location
            ...
            this.process = (0, child_process_1.spawn)(pythonPath, args, {
                cwd: workingDir,
                env,
